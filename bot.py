...
async def get_climb_percent(ctx, team):
    await ctx.defer()
    await ctx.respond(f"{team} climbed in {main.climb_percentage(int(team))}% of their matches")
...
async def get_auto_leave_percent(ctx, team):
    await ctx.defer()
    await ctx.respond(f"{team} left the start line in {main.auto_leave_percentage(int(team))}% of their matches")
...
async def summary(ctx, team):
    ...
    await ctx.respond(
        ...
        f"climbs: {main.climb_percentage(int(team))}% of the time\n" +
