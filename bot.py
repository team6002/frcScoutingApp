...
async def get_climb_percent(ctx, team):
    await ctx.defer()
    await ctx.respond(f"{team} climbed {main.get_percentage(int(team), "climb")}")
...
async def get_auto_leave_percent(ctx, team):
    await ctx.defer()
    await ctx.respond(f"{team} left the start line {main.get_percentage(int(team), "auto_leave")}")
